...
* Trade shares are seeded with the empirical share ratio
  ...
  ``pi`` (the previous quarter's converged shares).

``expenditure_tvf.m`` and ``GMC_tvf.m`` are mathematically identical to
the Phase 1 ``expenditurenew`` and ``GMCnew`` (only argument names
differ), so we import those directly.
"""
...
def P_h_om_tvf(
    ...
    """Fixed-point on (p_hat, x_hat) given a factor-price guess om.

    Same structure as the static ``P_h_om`` but with the dynamic targets
    ``om0`` and ``pi_tilde0 / pi_tilde1`` baked in. ``pi`` is the
    previous quarter's converged trade-share matrix.
    """
    G_3d = G.reshape(N, J, J)                           # [n, source, dest]
    LT_col = _inv_theta_per_jn(T, N)
    # pi_k[j*N + dest, source] = (pi_tilde1 / pi_tilde0) · pi · κ^(-1/θ)
    # NaN from 0/0 cells in the target ratio → 0.
    with np.errstate(invalid="ignore", divide="ignore"):
        pi_k = (pi_tilde1 / pi_tilde0) * pi * (kappa_hat ** (-1.0 / LT_col))
    pi_k = np.where(np.isnan(pi_k), 0.0, pi_k)
    ...
    while it <= maxit and pfmax > tol:
        # log x_hat_{k,n} = γ_{k,n} log(om/om0)_{k,n} + Σ_s G_{n,s,k} log p_hat_{s,n}
        lc = gamma * np.log(om / om0) + np.einsum("nsk,sn->kn", G_3d, np.log(p_hat0))
        x_hat = np.exp(lc)

        # p_hat_{j,d} = (Σ_m pi_k_{j,d,m} · A^(γ/θ) · x_hat^(-1/θ))^(-θ_j)
...
def Dinprime_tvf(
    ...
    """New bilateral trade shares with the data-target shift baked in."""
    T_col = T.reshape(-1, 1)
    LT_col = _inv_theta_per_jn(T, N)
    with np.errstate(invalid="ignore", divide="ignore"):
        pi_k = (pi_tilde1 / pi_tilde0) * pi * (kappa_hat ** (-1.0 / LT_col))
